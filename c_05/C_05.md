...
int main(void) {
	...
	scanf_s("%d %c %d", &x, &op, 1, &y);  // scanf_s 함수 호출 시, %d 서식지정자로 입력받은 값이 저장될 변수 x와 y가 char 타입의 op 다음에 위치하여 작동하지 않는 에러 확인

	//즉 scanf_s 함수가 op 변수에 입력받은 연산자를 저장한 후, 버퍼에 남아 있는 공백 문자(\n, \t, )를 읽어들여 y 변수에 저장하게 되므로 에러가 발생함. 
	//%d %c %d 사이에 공백 추가로 오류 해결함.
	switch (op)
	{
	case '+':
		result = x + y;
